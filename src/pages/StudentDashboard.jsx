      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-slate-200">
          {[
            ...
            <div key={complaint.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex-1 min-w-0">
                <p className="font-medium text-slate-950 truncate">{complaint.title}</p>
                <p className="text-sm text-slate-500">{complaint.id} • {complaint.date}</p>
              </div>
              <span
